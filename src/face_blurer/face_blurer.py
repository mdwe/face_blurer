...
def lambda_handler(event, context):
    try:
        ...
        if len(faces_boundaries) == 0:
            return {"statusCode": 200, "body": "Faces have been not detected"}
        # Blur faces form
        blurer = Blurer(source_image, faces_boundaries, destination_bucket)
        blurer.blur()

        return {"statusCode": 201, "body": f"Blurred image {source_image}"}
    except Exception:
...
def detect_faces(source_image: SourceImage, client=boto3.client("rekognition")) -> dict:
    ...
    response = client.detect_faces(
        Image={
            "S3Object": {
                "Bucket": source_image.bucket_name,
                "Name": source_image.file_key,
            }
        },
        Attributes=["ALL"],
    )
    ...
    logger.info(
        f"Detected {len(response['FaceDetails'])} faces for photo {source_image.file_key}"
    )
...
def upload_image_to_s3(
    ...
    try:
        obj = s3.Object(bucket_name=destination_bucket, key=destination_key)
        s3_response = obj.put(Body=buffer)
        logger.info(f"File saved at {destination_bucket}/{destination_key}")
        return s3_response
    except Exception as ex:
        logger.error(
            f"Exeption on upload file to {destination_bucket}/{destination_key}: {ex}"
        )
        raise ex
...
class Blurer:
    def __init__(
        self, source_image: SourceImage, faces_boundaries: list, destination_bucket: str
    ):
    ...
    def _load_image(self) -> str:
        ...
        obj = s3.Object(bucket_name=self.bucket, key=self.photo,)
    ...
    def _blur_boundary(self, face: dict) -> None:
        ...
        boundaries_box = (
            ...
            floor(
                self.img.size[0] * boundaries["Left"]
                + self.img.size[0] * boundaries["Width"]
            ),
            floor(
                self.img.size[1] * boundaries["Top"]
                + self.img.size[1] * boundaries["Height"]
            ),
        )
        ...
        pixelated_image_small = crop_img.resize(
            (new_width, new_hight), resample=Image.BILINEAR
        )
        pixelated_image = pixelated_image_small.resize(
