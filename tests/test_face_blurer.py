...
from face_blurer.face_blurer import (
    SourceImage,
    upload_image_to_s3,
    lambda_handler,
    detect_faces,
...
def origin_bucket_with_sample_image(origin_bucket):
    ...
    image_path = (
        Path(os.path.dirname(os.path.abspath(__file__))) / "sample_images" / "faces.jpg"
    )
    for file_path, content in [("faces.jpg", open(image_path, "rb"))]:
        add_file_to_bucket(file_path=file_path, content=content)


def test_event_processing_with_faces(
    env_variables, trigger_event, source_image, mocker
):
...
def test_event_processing_with_exception(trigger_event, mocker):
    mocker.patch(
        "face_blurer.face_blurer.detect_faces", side_effect=Exception("mocked error")
    )
    response = lambda_handler(trigger_event, {})
...
def test_save_file_in_destination_bucket(destination_bucket, tmp_dir):
    source_image_path = "test/faces.jpg"
    image_path = (
        Path(os.path.dirname(os.path.abspath(__file__))) / "sample_images" / "faces.jpg"
    )
    image = Image.open(image_path)
    s3_response = upload_image_to_s3(
        image, "face-blurer-destination", source_image_path
    )
    assert s3_response.get("ResponseMetadata").get("HTTPStatusCode") == 200

    client = boto3.client("s3")
    results = client.list_objects(
        Bucket="face-blurer-destination", Prefix=source_image_path
    )
...
def test_detect_faces(
    source_image, origin_bucket_with_sample_image, rekognition_response, mocker
):
